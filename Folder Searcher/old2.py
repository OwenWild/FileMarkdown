...
- Samples a much larger share of *eligible* files per folder (default 65%),
...
class OllamaClient:
    def __init__(self, base_url: str, model: str, timeout: int = 240, keep_alive: str = "30m") -> None:
        if requests is None:
    ...
    def generate_json(
        ...
        text = self.generate_text(prompt=prompt, system=system, images=images, schema=schema, temperature=temperature)
        try:
            return json.loads(text)
        except json.JSONDecodeError as exc:
            raise LocalAIError(f"Model did not return valid JSON. First 400 chars: {text[:400]!r}") from exc
...
def generate_root_narrative(ai: Optional[OllamaClient], payload: Dict[str, Any]) -> Dict[str, Any]:
    if ai is None:
        return {
            ...
            "top_tags": [item["label"] for item in payload["child_folders"][:5]],
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--max-evidence-files-per-folder", type=int, default=180, help="Hard cap on evidence files per top-level folder")
...
def main() -> int:
    ...
    if not args.disable_ai:
        try:
            ...
            print(f"[2/6] Local AI enabled via Ollama at {args.ollama_url} using model {args.model}")
        except Exception as exc:
