...
class OllamaClient:
    def __init__(self, base_url: str, model: str, timeout: int = 240, keep_alive: str = "30m", num_ctx: int = 32768, think: bool = False) -> None:
        if requests is None:
    ...
    def _candidate_models(self) -> List[str]:
        models = [self.model]
        # Thinking-tag variants are great interactively, but for automation they can spend
        # the whole token budget in the `thinking` field and return an empty final answer.
        # If the chosen tag ends with or contains `-thinking`, retry once with the sibling
        # non-thinking tag before giving up.
        if "-thinking" in self.model:
            sibling = self.model.replace("-thinking", "")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--think", action="store_true", help="Allow thinking traces on supported models. Off by default for automation.")
...
def main() -> int:
    ...
    if not args.disable_ai:
        try:
            ai = OllamaClient(base_url=args.ollama_url, model=args.model, timeout=args.ai_timeout, num_ctx=args.num_ctx, think=args.think)
            print(f"[2/6] Local AI enabled via Ollama at {args.ollama_url} using model {args.model} (num_ctx={args.num_ctx}, think={args.think})")
        except Exception as exc:
